...
function ai_chat_enable(){
  $get_data= ai_chat_get();
...
function ai_chatboat_option(){
  ...
  $nonce = isset($_REQUEST['_nonce'])?$_REQUEST['_nonce']:false;
    ...
    if ( ! wp_verify_nonce( $nonce, 'ai-chat-nonce' ) ) {        
                return false;
    }else{
        if(isset($_POST['aichatboat_upadte']) && !empty($_POST['aichatboat_upadte'])){
            ...
            if(ai_chat_get()){
                update_option( 'ai-chat', $api );
            }else{
                add_option( 'ai-chat', $api );
            }
        }
    }
}
...
// api api key
function ai_chat_get($key=false){
   $open_api =  get_option( 'ai-chat' );
   $option = isset($open_api[$key])?$open_api[$key]:$open_api;
   return $option;
}

function ai_chat_backend_form(){
    $nonce = wp_create_nonce( 'ai-chat-nonce' );
    ai_chatboat_option();
...
?>
   <h2><?php _e('AI Chatboat (Open AI ChatGPT)','ai-chat'); ?></h2>
    <form method="post" action="<?php echo admin_url('admin.php?page=ai-chat&_nonce='.$nonce); ?>">
    <ul class="wrapper">
        <li class="form-li">
          <div class="form-row">
          <label class="ai-label" for="open_api_key"><?php _e('Open AI ChatGPT API Key','ai-chat'); ?></label>
        ...
        <li class="form-row">
          <label class="ai-label" for="open_api_key"><?php _e('Chat Popup Visible','ai-chat'); ?></label>
          <label for="adminuser">
...
function ai_chat_live_chat() {
   ...
   <div class="form-container ">
     <div class="ai-header" onclick="openForm()"> <img class="ai-head-img" id="user-0" src="<?php echo esc_url(AI_CHAT_URL); ?>images/bot-chat.png"><h2><?php _e('Live AI Chatbot','ai-chat'); ?></h2></div>
      <section class="chat-window" id="chat-window">
       ...
       <button id="chatClick">
       <img class="chat-arrow" src="<?php echo esc_url(AI_CHAT_URL); ?>images/arrow-icon.png" style="display:block;">
                 </button>
        <div class="ai-chat-powerd-by"><?php _e('Powerd by','ai-chat'); ?></a> <a href="https://themehunk.com" target="_blank"><?php _e('ThemeHunk','ai-chat'); ?></a></div>
     </form>
 </div>
 ...
 add_action( 'wp_footer', 'ai_chat_enable', 100 );